...
describe("ProjectForm", () => {
  ...
  const mockFormData = {
    clients: [{ id: "1", name: "Test Company" }],
    teamMembers: [{ id: "1", name: "Test User", role: "Manager" }],
    taxReturns: [{ id: "1", name: "2023 Tax Return" }],
    formProgress: 0,
    ...
    getTaxReturnOptions: () => [{ value: "1", label: "2023 Tax Return" }],
    form: {
      watch: jest.fn(),
      setValue: jest.fn(),
  ...
  it("handles template selection", async () => {
    ...
    expect(mockFormData.handleTemplateChange).toHaveBeenCalledWith(
      mockTemplate.id,
    );
  });
  ...
  describe("Task Management", () => {
    ...
    it("validates task dependencies before submission", async () => {
      ...
      expect(toast.error).toHaveBeenCalledWith(
        "Please fix task dependency errors",
      );
  ...
  describe("Team Member Management", () => {
    ...
    it("validates team member assignments in tasks", async () => {
      ...
      expect(
        screen.getByText(/invalid team member assignment/i),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Service Type Management", () => {
    ...
    it("validates service-specific fields before submission", async () => {
      ...
      expect(toast.error).toHaveBeenCalledWith(
        "Please fill in all required fields for the selected service type",
      );
      expect(mockSubmitProject).not.toHaveBeenCalled();
    });
  });
