...
function calculateTaskTypeScore(task: Task): number {
  ...
  if (task.category) {
    const categoryWeight = CATEGORY_WEIGHTS[task.category.toLowerCase()] || CATEGORY_WEIGHTS.other
...
function calculateDependenciesScore(task: Task, projects?: Project[]): number {
  ...
  if (projects && task.project_id) {
    ...
    if (project && project.due_date) {
      ...
      const now = new Date()
      const daysUntilProjectDue = Math.ceil((projectDue.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
